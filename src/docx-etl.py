...
# def has_existing_bill_text_supabase_records(supa_con, bill_no, printer_no, leg_body):
#     #existing_bill_records = supa_con.table('Revisions').select("*", count="exact").eq("bill_id", bill_no).eq("printer_no", printer_no).eq("legislative_body", leg_body).execute()
#     #if existing_bill_records.count > 0: return True
#     #else: return False
#     return
...
# def extract_and_upload_senate_bill_text(supa_con):
#     senate_bill_records_missing_text = supa_con.table("Revisions").select("bill_id, printer_no, legislative_body").eq("full_text", None)
#     for record in senate_bill_records_missing_text:
#         bill_path = dowload_bill_doc_file(record["link"])
#         bill_docx_path = convert_doc_to_docx(bill_path)
#         bill_text = extract_law_text_from_docx(bill_docx_path)
#         os.remove(bill_path)
#         os.remove(bill_docx_path)

#         supa_con.table('Revisions').update({ "full_text": bill_text }).eq("bill_id", record["bill_id"]).eq("printer_no", record["printer_no"]).eq("legislative_body", record["legislative_body"]).execute()

#     # sen_rss = feedparser.parse(senate_rss_feed)
    
#     #grab records with null values
...
if __name__ == "__main__":
    ...
    # extract_and_upload_senate_bill_text(supabase_connection)
